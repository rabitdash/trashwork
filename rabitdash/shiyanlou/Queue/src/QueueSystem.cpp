...
QueueSystem::QueueSystem (int total_service_time, int window_num):total_service_time (total_service_time), window_num (window_num),
...
QueueSystem::simulate (int simulate_num)
{
  ...
  avg_customers =
    (double) total_customer_num / (total_service_time * simulate_num);
}
  ...
  if (idleIndex >= 0)
    {
      ...
      Event temp_event (current_event->occur_time + customer->duration,
			idleIndex);
      event_list.orderEnqueue (temp_event);
    }
  delete customer;
...
QueueSystem::customerDeparture ()
{
  if (current_event->occur_time < total_service_time)
      ...
      if (customer_list.length ())
	{
	  ...
	  Event temp_event (current_event->occur_time +
			    customer->duration, current_event->event_type);
	  event_list.orderEnqueue (temp_event);
	  delete customer;
	}
      else
